...
## What the weight report can and cannot say about the method

A weight-fidelity report describes the geometry of the model it loaded, not the recipe that produced it. mlx-lm's quantizers all write the same kind of `quantization` block into `config.json`: a top-level `bits` and `group_size` (and sometimes `mode`), plus optional per-module entries — a dict of `bits`/`group_size` for a module quantized differently, `True` for a module quantized at the top-level defaults, `False` for one left alone. Repos written by the current `mlx_lm.convert` carry the block twice, under `quantization` and `quantization_config`; repos written by other mlx-lm tools or older conversions, including the ladder's `-4bit-AWQ` repo, carry only `quantization`, and the probe reads `quantization` first either way. Nothing in that block says whether the scales were learned (`mlx_lm.dwq`), the weights pre-scaled (`mlx_lm.awq`), or the per-layer bit widths chosen by sensitivity (`mlx_lm.dynamic_quant`): a repo published as DWQ has a config indistinguishable from a plain 4-bit one, and a repo published as AWQ simply lists every module with the geometry it ended up with. The report therefore never labels a method. The repo id may say `-DWQ`; the tool did not check that, and says so in a standing note.

What it does report is measured from the loaded model. `quant_geometry` lists each distinct (bits, group size) combination and how many modules use it; `quant_n_full_precision` counts modules that could have been quantized (linears, embeddings) but were not; `quant_precision` is `uniform` when at most one bit width is in use and `mixed` otherwise — a model quantized at 4 bits with a group-32 embedding is uniform, one with 4- and 5-bit layers is mixed. These are read from the modules themselves rather than from the config entries, because the entries describe how the converter wrote the file, not the model: a block written by `mlx_lm.awq` lists norms and rotary-embedding modules as `False` even though they hold nothing quantizable, and mlx-lm's converter silently skips any module whose width is not a multiple of the group size without recording it. `quant_bits` and `quant_group_size` stay as the declared top-level defaults.

`quant_bits_per_weight` is the effective precision: the loaded model's array bytes times eight, divided by its parameter count, using the same definition `mlx_lm.convert` prints — scale and bias storage included, unquantized parameters included. A uniform 4-bit, group-64 model comes out at 4.50 (4 bits plus two fp16 values per 64 weights). It is a per-parameter view of the model's size, not a second cost axis beside the on-disk bytes the ranking uses.
...
The `kv` probe quantizes the layers it can and leaves the rest at full precision. The reported drift is the cost of quantizing that subset. The report records the split — how many layers were quantized, and the type and count of the ones skipped — and refuses the model only when no layer can be quantized at all (a pure state-space model has no KV cache to measure). If the model's forward cannot run over a mixed quantized and full-precision cache, the probe flags it with a clear error before scoring any chunk, rather than failing part-way through a run.

This is a partial measurement, and what it corresponds to in mlx-lm depends on which layer type was skipped. For a sliding-window hybrid it is a configuration mlx-lm never reaches. In mlx-lm 0.31.3 a sliding-window layer's `RotatingKVCache` defines `to_quantized`, but the method raises `NotImplementedError`, and the conversion gate in `generate.py` checks only that the method exists and that the layer has reached the start offset, never whether the method works, so the run crashes at the conversion point instead of skipping the layer. The `mlx_lm.generate` command line starts conversion at token 5000 by default, prompt tokens included, so a long prompt trips the crash during prefill and a shorter one trips it once the cache reaches that length; the Python API defaults the start to 0 and crashes on the first prefill chunk. An attention plus state-space hybrid is different. Its state-space layers define no `to_quantized` at all, so the same gate passes over them without a message, and mlx-lm does generate with that mixed cache. The probe's partial number answers a narrower question in both cases: if only the quantizable layers were quantized, this is what it would cost. For a sliding-window hybrid that is a cost no shipped configuration pays; for a state-space hybrid it is the cost mlx-lm's own generate path pays once conversion starts. Read it as the quantizer's effect on the layers it reaches, not as the model's deployed KV-quant cost.

Partial coverage runs in stress mode only. Deployment mode converts the whole stored prefix at the boundary, which fails on a sliding-window layer, so a partial model is refused there rather than measured. The quantizer-only control lane is refused for the same reason. It runs in the `kv` command; `compare kv` does not rank a partial model, because a partial cache's cost is not comparable to a fully-quantized one on the same frontier.
