...
This is a partial measurement, and it describes a configuration mlx-lm does not produce. Pass `kv_bits` to a hybrid model through mlx-lm's own generation path and the outcome depends on the cache class of each layer. A sliding-window layer's `RotatingKVCache` defines `to_quantized`, but the method raises `NotImplementedError`; the conversion gate in `generate.py` checks only that the method exists, so the run crashes at the conversion point instead of skipping the layer. The generate command starts conversion at token 5000 by default, so the crash shows up only on long generations, which is why it is rarely reported. The state-space and chunked cache classes define no `to_quantized` at all, so the same gate passes over them and those layers stay full-precision without a message. Verified against mlx-lm 0.31.3 (`models/cache.py`, `generate.py`). The probe's partial number answers a narrower question: if only the quantizable layers were quantized, this is what it would cost. Read it as the quantizer's effect on the layers it reaches, not as the model's deployed KV-quant cost.
