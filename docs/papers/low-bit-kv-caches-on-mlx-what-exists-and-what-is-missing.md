...
| stock 8/4-bit vs turboquant-mlx's uniform-bit 4/3-bit cache, ranked memory-normalized on Llama-3.2-1B, stress mode, 100 chunks: turboquant:3 8.2 KB/token (KL mean 0.4229, bad); stock 4:64 9.2 KB/token (KL mean 0.1477, bad, dominated by turboquant:4); turboquant:4 9.2 KB/token (KL mean 0.0825, bad); stock 8:64 17.4 KB/token (KL mean 0.0004, marginal) | **superseded by the 0.7.0 row below** — stock's number here is its bundled deployed-path KL, ranked directly against TurboQuant's already-quantizer-only number; see that row for the same comparison on one footing | [`_artifacts/samples/compare/kv-llama-3.2-1b-methods.md`](https://github.com/IonDen/mlx-quant-fidelity/blob/v0.6.0/_artifacts/samples/compare/kv-llama-3.2-1b-methods.md) |
| Re-footed on 0.7.0: the same Llama-3.2-1B comparison ranked with every method on quantizer-only drift, stock included. turboquant:3 8.2 KB/token (KL mean 0.4229, bad); stock 4:64 9.2 KB/token (KL mean 0.1485 quantizer-only, 0.1477 on its own bundled deployed path — the quantized-attention kernel's own contribution measures close to nil at this configuration and scale; bad, dominated by turboquant:4); turboquant:4 9.2 KB/token (KL mean 0.0825, bad); affine:8:4 13.3 KB/token (KL mean 0.0120, on the frontier — an independent per-side K/V bit width with no shipped runtime, measured quantizer-only); stock 8:64 17.4 KB/token (KL mean 0.0004, marginal); turboquant-vonly:4 37.4 KB/token (KL mean 0.0078, dominated by stock 8:64 — the pinned port's V-only cache stores an unused fp16 copy of V, so its stored bytes exceed a plain fp16 cache's even though V itself compresses) | measured in this repository; every number is quantizer-only, including stock's, via a dequantize-then-standard-SDPA control lane run alongside its bundled path | [`_artifacts/samples/compare/kv-llama-3.2-1b-methods.md`](https://github.com/IonDen/mlx-quant-fidelity/blob/v0.7.0/_artifacts/samples/compare/kv-llama-3.2-1b-methods.md) |
...
*Prepared 2026-07-25. Last updated 2026-08-27. Denis Ineshin.*
